...
SRC_URI="https://download:${PASSENGER_ENTERPRISE_DOWNLOAD_KEY}@www.phusionpassenger.com/orders/download?dir=${PV}&file=passenger-enterprise-server-${PV}.tar.gz -> ${P}.tar.gz"
...
nginx-module-setup() {
	if [[ -z ${PASSENGER_ENTERPRISE_DOWNLOAD_KEY} ]]; then
		eerror "You must specify PASSENGER_ENTERPRISE_DOWNLOAD_KEY variable"
		eerror "in /etc/portage/env/${CATEGORY}/${PN} file."
		eerror "set it's value to your download key obtained from Phusion"
		die "PASSENGER_ENTERPRISE_DOWNLOAD_KEY wasn't set"
	fi
