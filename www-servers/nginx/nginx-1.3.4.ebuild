...
SRC_URI="http://nginx.org/download/${P}.tar.gz
	...
	nginx_modules_http_supervisord? ( ${HTTP_SUPERVISORD_MODULE_URI}.tar.gz )
	nginx_modules_http_auth_request? ( ${HTTP_AUTH_REQUEST_MODULE_URI}.tar.gz )
	nginx_modules_http_slowfs_cache? ( ${HTTP_SLOWFS_CACHE_MODULE_URI}.tar.gz )
...
# http_passenger
	if use nginx_modules_http_passenger; then
		http_enabled=1
		myconf+=" --add-module=${HTTP_PASSENGER_MODULE_PV}/ext/nginx"
	fi
