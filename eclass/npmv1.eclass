	...
	if has "${EAPI:-0}" 5 6 7 8; then
		...
		if [[ -z "${SRC_URI}" && -z "${EGIT_REPO_URI}" ]] ; then
			if [[ -n "${NPM_GITHUP_MOD}" ]] ; then
				...
				SRC_URI="https://registry.npmjs.org/${NPM_PKG_NAME}/-/${NPM_PKG_NAME}-${PV}.tgz"
			fi
		fi
	else
...
npmv1_src_compile() {
	if [[ -z "${NPM_GYP_BIN}" ]] ; then
		NPM_GYP_BIN="${EPREFIX}/usr/$(get_libdir)/node_modules/npm/bin/node-gyp-bin/node-gyp"
	fi
...
npmv1_src_install() {
	...
	if [[ -z "${NPM_PACKAGEDIR}" ]] ; then
		NPM_PACKAGEDIR="${EPREFIX}/usr/$(get_libdir)/node_modules/${NPM_PKG_NAME}"
	fi
...
def_node_path="${EPREFIX}/usr/$(get_libdir)/node_modules/"
