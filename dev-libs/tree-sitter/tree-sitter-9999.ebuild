...
IUSE="ts-cli"
# ^ Actually, I'd like to use just "cli" flag, but it is globally
# enabled in linux profile, so it will raise a dependency on
# dev-lang/rust (or bin) for eveyone, which is not a good idea...
...
src_unpack() {
	if [[ ${PV} == *9999* ]]; then
		...
		cargo_src_unpack
		# ^ covers "default" too, so can be left that way irrelevant
		# to state of ts-cli flag
	fi
}
