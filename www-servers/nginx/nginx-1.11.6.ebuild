...
HTTP_REDIS_MODULE_SHA="7aa3470e6cba18252ebf4955e1204f08e1d1a087"
...
#HTTP_LUA_MODULE_SHA="91fc078c44cd76ab173ad4e538ff8fd630d71740"
...
STREAM_LUA_MODULE_SHA="e527417c5d04da0c26c12cf4d8a0ef0f1e36e051"
...
HTTP_DRIZZLE_MODULE_SHA="df0f596553656a809a489e4991c57565e215ed43"
...
HTTP_MEMC_MODULE_SHA="cebe3217a3b4944bf39e1d089fff952cb9f708a8"
...
HTTP_LUA_UPSTREAM_MODULE_PV="0.06"
...
HTTP_SECURITY_MODULE_SHA="bb577950bf983811ff1892e87d815a1909c0b96b"
...
HTTP_NJS_MODULE_SHA="19758fdcd554"
...
#	)
	...
	if use nginx_modules_http_memc; then
		pushd "${HTTP_MEMC_MODULE_WD}" &>/dev/null
		epatch "${FILESDIR}/${P}-http_memc-${HTTP_MEMC_MODULE_SHA}.patch"
		popd &>/dev/null
	fi

	if use nginx_modules_http_drizzle; then
		pushd "${HTTP_DRIZZLE_MODULE_WD}" &>/dev/null
		epatch "${FILESDIR}/${P}-http_drizzle-${HTTP_DRIZZLE_MODULE_SHA}.patch"
		popd &>/dev/null
	fi

	if use nginx_modules_http_postgres; then
		pushd "${HTTP_POSTGRES_MODULE_WD}" &>/dev/null
		epatch "${FILESDIR}/${P}-http_postgres-${HTTP_POSTGRES_MODULE_SHA}.patch"
		popd &>/dev/null
	fi

	if use nginx_modules_http_redis; then
		pushd "${HTTP_REDIS_MODULE_WD}" &>/dev/null
		epatch "${FILESDIR}/${P}-http_redis-${HTTP_REDIS_MODULE_SHA}.patch"
		popd &>/dev/null
	fi
